...
const SECTIONS: Array<{ key: string; label: () => string }> = [
  ...
  { key: "update", label: () => t("config.sectionUpdates") },
  { key: "agents", label: () => t("config.sectionAgents") },
  { key: "auth", label: () => t("config.sectionAuth") },
  { key: "channels", label: () => t("config.sectionChannels") },
  { key: "messages", label: () => t("config.sectionMessages") },
  { key: "commands", label: () => t("config.sectionCommands") },
  { key: "hooks", label: () => t("config.sectionHooks") },
    ...
    <div class="config-layout">
      ...
      <aside class="config-sidebar">
        <div class="config-sidebar__header">
          <div class="config-sidebar__title">${t("tabs.config")}</div>
          <span
            class="pill pill--sm ${
              validity === "valid" ? "pill--ok" : validity === "invalid" ? "pill--danger" : ""
            }"
            >${validity === "valid" ? t("config.valid") : validity === "invalid" ? t("config.invalid") : t("config.unknown")}</span
          >
        </div>
                  ...
                  <span class="config-changes-badge"
                    >${
                      props.formMode === "raw"
                        ...
                        : t("config.unsavedCount", { count: diff.length }) + (diff.length !== 1 ? "s" : "")
                    }</span
                  >
                `
                : html`
            ...
            ? html`
              <details class="config-diff">
                <summary class="config-diff__summary">
                  <span
                    >${t("config.viewChanges", { count: diff.length })}${diff.length !== 1 ? "s" : ""}</span
                  >
                  <svg
