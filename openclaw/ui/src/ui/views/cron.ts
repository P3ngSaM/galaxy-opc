...
export function renderCron(props: CronProps) {
  ...
  const selectedRunTitle = selectedJob?.name ?? props.runsJobId ?? t("cron.history.selectJob");
  ...
  return html`
    ...
    <section class="card" style="margin-top: 18px;">
      <div class="card-title">${t("cron.history.title")}</div>
      <div class="card-sub">${t("cron.history.subtitle", { job: selectedRunTitle })}</div>
      ${
        props.runsJobId == null
          ? html`
              <div class="muted" style="margin-top: 12px">${t("cron.history.selectHint")}</div>
            `
          : orderedRuns.length === 0
            ? html`
                <div class="muted" style="margin-top: 12px">${t("cron.history.empty")}</div>
              `
            : html`
              <div class="list" style="margin-top: 12px;">
    ...
    ${
      delivery
        ? html`<div class="cron-job-detail">
            <span class="cron-job-detail-label">${t("cron.job.deliveryLabel")}</span>
  ...
  return html`
    <div class="cron-job-state">
      <div class="cron-job-state-row">
        <span class="cron-job-state-key">${t("cron.state.status")}</span>
        <span class=${`cron-job-status-pill ${statusClass}`}>${status}</span>
      </div>
      <div class="cron-job-state-row">
        <span class="cron-job-state-key">${t("cron.state.next")}</span>
        <span class="cron-job-state-value" title=${formatMs(nextRunAtMs)}>
          ${formatStateRelative(nextRunAtMs)}
        </span>
      </div>
      <div class="cron-job-state-row">
        <span class="cron-job-state-key">${t("cron.state.last")}</span>
  ...
  return html`
    <div class="list-item">
      ...
      <div class="list-meta">
        ...
        ${
          chatUrl
            ? html`<div><a class="session-link" href=${chatUrl}>${t("cron.run.openChat")}</a></div>`
            : nothing
        }
