...
export function renderAgentTools(params: {
  ...
  const profileSource = agentTools.profile
    ? t("agents.tools.sourceAgentOverride")
    : globalTools.profile
      ? t("agents.tools.sourceGlobalDefault")
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          ...
          <div class="card-sub">
            ...
            <span class="mono">${enabledCount}/${toolIds.length}</span> ${t("agents.tools.enabledSuffix")}
          </div>
        </div>
        <div class="row" style="gap: 8px;">
          ...
          >
            ${params.configSaving ? t("agents.saving") : t("agents.save")}
          </button>
        </div>
      </div>
        ...
        !params.configForm
          ? html`
              <div class="callout info" style="margin-top: 12px">
                ${t("agents.tools.calloutNoConfig")}
              </div>
            `
          : nothing
      }
      ${
        hasAgentAllow
          ? html`
              <div class="callout info" style="margin-top: 12px">
                ${t("agents.tools.calloutHasAllowlist")}
              </div>
            `
          : nothing
      }
      ${
        hasGlobalAllow
          ? html`
              <div class="callout info" style="margin-top: 12px">
                ${t("agents.tools.calloutGlobalAllow")}
              </div>
            `
          : nothing
      }
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        ...
        <div class="row" style="gap: 8px;">
          ...
          <button class="btn btn--sm" ?disabled=${params.loading} @click=${params.onRefresh}>
            ${params.loading ? t("agents.loading") : t("common.refresh")}
          </button>
          <button
            class="btn btn--sm primary"
            ?disabled=${params.configSaving || !params.configDirty}
      ...
      ${
        !params.configForm
          ? html`
              <div class="callout info" style="margin-top: 12px">
                ${t("agents.skills.calloutNoConfig")}
              </div>
            `
          : nothing
      }
      ${
        usingAllowlist
          ? html`
              <div class="callout info" style="margin-top: 12px">${t("agents.skills.calloutUsingAllowlist")}</div>
            `
          : html`
              <div class="callout info" style="margin-top: 12px">
                ${t("agents.skills.calloutAllEnabled")}
              </div>
            `
      }
      ${
        !reportReady && !params.loading
          ? html`
              <div class="callout info" style="margin-top: 12px">
                ${t("agents.skills.calloutNotLoaded")}
              </div>
            `
          : nothing
      }
  ...
  return html`
    <div class="list-item agent-skill-row">
      <div class="list-main">
        ...
        ${
          missing.length > 0
            ? html`<div class="muted" style="margin-top: 6px;">${t("skills.missing")} ${missing.join(", ")}</div>`
            : nothing
        }
        ${
          reasons.length > 0
            ? html`<div class="muted" style="margin-top: 6px;">${t("skills.reason")} ${reasons.join(", ")}</div>`
            : nothing
        }
      </div>
      <div class="list-meta">
