    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          ...
          <div class="card-sub">${t("nodes.pairedAndLinks")}</div>
        </div>
      ...
      <div class="list" style="margin-top: 16px;">
        ${
          props.nodes.length === 0
            ? html`
                <div class="muted">${t("nodes.noNodes")}</div>
              `
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        ${
          tokens.length === 0
            ? html`
                <div class="muted" style="margin-top: 6px">${t("nodes.tokensNone")}</div>
              `
            : html`
...
function renderTokenRow(deviceId: string, token: DeviceTokenSummary, props: NodesProps) {
  const status = token.revokedAtMs ? t("nodes.tokenRevoked") : t("nodes.tokenActive");
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        <div>
          ...
          <div class="card-sub">
            ${t("nodes.execNodeBindingSubtitle")}
          </div>
        </div>
        <button
          class="btn"
      ...
      ${
        !state.ready
          ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            <div class="muted">${t("nodes.loadConfigHint")}</div>
            ...
            <div class="list" style="margin-top: 16px;">
              <div class="list-item">
                <div class="list-main">
                  ...
                  <div class="list-sub">${t("nodes.defaultBindingSubtitle")}</div>
                </div>
                <div class="list-meta">
                  <label class="field">
                    <span>${t("nodes.node")}</span>
                    <select
                  ...
                  ${
                    !supportsBinding
                      ? html`
                          <div class="muted">${t("nodes.noExecNodes")}</div>
                        `
                      : nothing
                  }
                </div>
              ...
              ${
                state.agents.length === 0
                  ? html`
                      <div class="muted">${t("nodes.noAgents")}</div>
                    `
        ...
        <div class="list-sub">
          ...
          ${
            bindingValue === "__default__"
              ? `${t("nodes.usesDefault")} (${state.defaultBinding ?? t("nodes.any")})`
...
function renderNode(node: Record<string, unknown>) {
  ...
  const title =
    (typeof node.displayName === "string" && node.displayName.trim()) ||
    (typeof node.nodeId === "string" ? node.nodeId : t("nodes.unknown"));
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="chip-row" style="margin-top: 6px;">
          <span class="chip">${paired ? t("nodes.nodePaired") : t("nodes.nodeUnpaired")}</span>
          <span class="chip ${connected ? "chip-ok" : "chip-warn"}">
            ${connected ? t("nodes.nodeConnected") : t("nodes.nodeOffline")}
          </span>
