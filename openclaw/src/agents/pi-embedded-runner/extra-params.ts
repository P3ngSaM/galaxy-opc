...
export function applyExtraParamsToAgent(
  ...
  // Inject enable_thinking parameter for DashScope/Qwen models.
  // When params.enable_thinking is explicitly set to false, inject it into the
  // request payload to disable the model's chain-of-thought reasoning mode,
  // significantly reducing latency for models like qwen3.5-plus.
  if (typeof merged?.enable_thinking === "boolean") {
    const enableThinking = merged.enable_thinking;
    log.debug(`setting enable_thinking=${enableThinking} for ${provider}/${modelId}`);
